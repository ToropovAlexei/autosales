...
use crate::models::common::{FilterValue, ListQuery, Operator, OrderDir, Pagination, ScalarValue};

pub fn apply_list_query<'a>(qb: &mut QueryBuilder<'a, Postgres>, list: &'a ListQuery) {
    apply_filters(qb, list);
    apply_order_by(qb, list);
    apply_pagination(qb, &list.pagination);
}

pub fn apply_filters<'a>(qb: &mut QueryBuilder<'a, Postgres>, list: &'a ListQuery) {
    ...
    for filter in &list.filters {
        ...
        match (&filter.op, &filter.value) {
            ...
            (Operator::Eq | Operator::Ne, FilterValue::Scalar(scalar)) => {
                let op = if filter.op == Operator::Eq { "=" } else { "!=" };
                qb.push(" AND ")
                    .push_bind(field_col)
                    .push(" ")
                    .push_bind(op)
...
pub fn apply_order_by<'a>(qb: &mut QueryBuilder<'a, Postgres>, list: &'a ListQuery) {
