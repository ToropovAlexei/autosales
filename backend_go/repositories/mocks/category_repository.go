...
func (m *MockCategoryRepository) Update(category *models.Category, data models.Category) error {
	return m.Called(category, data).Error(0)
}
func (m *MockCategoryRepository) Delete(category *models.Category) error {
	return m.Called(category).Error(0)
}
...
func (m *MockCategoryRepository) FindByNameAndParent(name string, parentID *uint) (*models.Category, error) {
...
func (m *MockCategoryRepository) FindOrCreateByPath(path []string) (*models.Category, error) {
	args := m.Called(path)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
